...
The toolchain ships three standalone command-line tools that work on `.m1scr`
scripts without an editor. They are the same engines the editor plugins drive
over LSP, so what you see in CI matches what you see in your editor.
...
The three CLIs share one contract so scripting and CI behave predictably.
...
Every tool reports its own version (e.g. `m1-fmt 0.6.0`) and exits `0`. Use this
to pin tool versions in CI.
...
All three tools (and the LSP that backs the editors) read a shared, layered
configuration. Lowest layer first; later layers override earlier ones:
